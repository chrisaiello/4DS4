...
-Pin assignments
	...
	--Set "Location" of clock to PIN_N2
	--Set "Location" of reset to PIN_G26 (for using Push button 0 as the reset)
-Synthesis
...
-To start a new project with the newly created system described above:
	...
	--Now select project template, for the system created as described above, select "Hello World Small", since only 1536 bytes
...
-Quartus project creation (optional - only if you plan to modify the .qsys file - alternatively you can program the .sof file directly from the NIOS SDK environment by launching the Quartus programmer using Ctrl+8)
	--Create a Quartus project based on the provided .qsf file in the same way as in experiment1
